...
if "genre" not in st.session_state:
    st.session_state.genre = "멜로"


@st.dialog("의견을 말씀해주세요.")
def vote_dialog(vote_type):
    st.write(f"{vote_type} 하는 이유는 무엇입니까?")
    reason = st.text_input("그 이유는...", placeholder="의견을 입력해주세요.")
    if st.button("제출"):
        if reason.strip():
            st.success(f"{vote_type} 의견이 제출되었습니다.")
            st.write(f"입력한 의견: {reason}")
        else:
            st.warning("의견을 입력한 후 제출해주세요.")
